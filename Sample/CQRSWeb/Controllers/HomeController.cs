...
namespace CQRSWeb.Controllers
{
    public class HomeController : Controller
    {
        ...
        {
            await _commandSender.SendAsync(new CreateInventoryItem(Guid.NewGuid(), name), cancellationToken);
        ...
        {
            await _commandSender.SendAsync(new RenameInventoryItem(id, name, version), cancellationToken);
            return RedirectToAction("Index");
        }

        public async Task<ActionResult> Deactivate(Guid id, int version, CancellationToken cancellationToken)
        {
            await _commandSender.SendAsync(new DeactivateInventoryItem(id, version), cancellationToken);
            return RedirectToAction("Index");
        }
        ...
        {
            await _commandSender.SendAsync(new CheckInItemsToInventory(id, number, version), cancellationToken);
            return RedirectToAction("Index");
        }
        ...
        {
            await _commandSender.SendAsync(new RemoveItemsFromInventory(id, number, version), cancellationToken);
            return RedirectToAction("Index");
        }
    }
}
