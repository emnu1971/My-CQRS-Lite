...
namespace CQRSlite.Tests.Extensions.TestHelpers
{
        ...
        where TCommand : ICommand
    {
        ...
        public Specification()
        {
            ...
            {
                handler.HandleAsync(When(), new CancellationToken());
            }
        ...
        private async Task<TAggregate> GetAggregate()
        {
            try
            {
                return await Session.GetAsync<TAggregate>(Guid.Empty);
            }
    ...
    internal class SpecEventStorage : IEventStore
    {
        ...
        public Task SaveAsync(IEnumerable<IEvent> events, CancellationToken cancellationToken = default(CancellationToken))
        {
            ...
            return Task.WhenAll(events.Select(evt =>_publisher.PublishAsync(evt, cancellationToken)));
